...
/*
	...
	n명의 사람(0부터)이 존재함.
	이들 중에는 비밀을 가진 한 사람이 존재하고,
	이 사람들은 회의를 수행하는데, 회의를 통해서 사람의 비밀이 전파됨.
	모든 비밀은 실시간으로 전파되어 회의가 잡히는 즉시 비밀이 공유됨.
	동시에 여러사람과 회의가 가능함 -> 즉 늦게 시작해도 겹치기만 하면 비밀이 공유됨.

	최초에 0번이 firstPerson과 비밀을 0시에 공유함.
*/
#include <iostream>
#include <algorithm>
#include <vector>
...
using node = pair<int, int>;	// first : id , second : time
...
public:
	...
	{
		vector<int> visitTime(n, 2147483647);
		vector<vector<node>> graph(n);
		priority_queue<node*, vector<node*>, cmp_node> nodePQ;
		vector<int> result;
		...
		{
			graph[meet[0]].push_back(make_pair(meet[1], meet[2]));
			graph[meet[1]].push_back(make_pair(meet[0], meet[2]));
		}

		// traverse
		visitTime[0] = 0;
		result.push_back(0);
		nodePQ.push(&graph[0][0]);
		visitTime[firstPerson] = 0;
		result.push_back(firstPerson);
		nodePQ.push(&graph[firstPerson][0]);
		while (!nodePQ.empty())
		{
			// current node
			const int& curId = nodePQ.top()->first;
			const int& curTime = nodePQ.top()->second;
			// next node enque
			for (node& nextNode : graph[curId])
			{
				const int &nextId = nextNode.first;
				const int &nextTime = nextNode.second;
				// time check
				if (visitTime[nextId] > nextTime && curTime <= nextTime)
				{
					if (visitTime[nextId] == 2147483647)
						result.push_back(nextId);
					nodePQ.push(&nextNode);
					// update
					visitTime[nextId] = nextTime;
				}
			}
			nodePQ.pop();
		}
...
int main()
{
	...
	int n = , fp = ;
	vector<vector<int>> m = {{, , }, };
	rersult = test.findAllPeople(, , );
	// 1
	int n1 = 6, fp1 = 1;
	vector<vector<int>> m1 = {{1, 2, 5}, {2, 3, 8}, {1, 5, 10} };
	result = test.findAllPeople(n1, m1, fp1);
	// 2
	int n2 = 4, fp2 = 3;
	vector<vector<int>> m2 = {
		...
		{1,2,2},
		{0,3,3} };
	result = test.findAllPeople(n2, m2, fp2);
	*/
	// 3
	int n3 = 135, fp3 = 125;
	...
	;
	result = test.findAllPeople(n3, m3, fp3);
