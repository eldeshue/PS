...
int isCross(Line& l1, Line& l2)
{
	...
	int v1 =
	cross(vec1, std::make_pair(l2.first.X - l1.second.X, l2.first.Y - l1.second.Y))
	* cross(vec1, std::make_pair(l2.second.X - l1.second.X, l2.second.Y - l1.second.Y));

	Point vec2 = std::make_pair(l2.first.X - l2.second.X, l2.first.Y - l2.second.Y);
	int v2 =
	cross(vec2, std::make_pair(l1.first.X - l2.second.X, l1.first.Y - l2.second.Y))
	* cross(vec2, std::make_pair(l1.second.X - l2.second.X, l1.second.Y - l2.second.Y));
...
std::pair<long double, long double> calCrossCoordinate(const Line& l1, const Line& l2)
{
	Point vec1 = std::make_pair(l1.first.X - l1.second.X, l1.first.Y - l1.second.Y);
	Point vec2 = std::make_pair(l2.first.X - l2.second.X, l2.first.Y - l2.second.Y);
	...
	{
		long double a1 = -vec1.Y;
		long double b1 = vec1.X;
		long double c1 = vec1.X * l1.first.Y - vec1.Y * l1.first.X;
		long double a2 = -vec2.Y;
		long double b2 = vec2.X;
		long double c2 = vec2.X * l2.first.Y - vec2.Y * l2.first.X;
		long double det = a1 * b2 - a2 * b1;
		long double x = (b2 * c1 - b1 * c2) / det;
		long double y = (-a2 * c1 + a1 * c2) / det;
...
int main()
{
	...
	if (numOfIntersect != 0)
	{
		...
		if (numOfIntersect > 0)
		{
			...
			std::cout.precision(9);	// 소수점 아래 16자 까지 출력, fix가 없으면 유효숫자 16개
			std::cout << x << ' ' <<  y << '\n';
		}
	}
	else
