...
int main()
{
	...
	// get length of convex-hull
	double lenOfBarrier = 2.0 * M_PI * L;
	std::vector<Point> convexHull = grahamScan(coordinates);
	for (int i = 0; i < convexHull.size() - 1; ++i)
		lenOfBarrier += sqrt(dist(convexHull[i], convexHull[i + 1]));
	lenOfBarrier += sqrt(dist(convexHull.front(), convexHull.back()));
	std::cout << static_cast<ll>(round(lenOfBarrier));
}
