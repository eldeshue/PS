...
private:
	...
	Dinic() = delete;
	Dinic(Dinic const&) = delete;
	Dinic& operator=(Dinic const&) = delete;
		...
		for (int& i = next_node_idx_[cur_node]; i < graph_[cur_node].size(); ++i)
		{
			...
			if (level_[next_node] > level_[cur_node] && residual_flow > 0)
			{
				...
				if (aug_flow > 0)
				{
					...
					flow_[next_node][cur_node] -= aug_flow;
					return aug_flow;
				}
			}
		}
...
public:
	Dinic(Node const source, Node const sink,
...
int main()
{
	...
	// solve
	Dinic solver(source, sink, graph, capacity);
	if (input_flow == output_flow && solver.GetMaxFlow() == input_flow)
	{
		std::cout << "1\n";
		for (int r = 1; r <= N; ++r)
		{
